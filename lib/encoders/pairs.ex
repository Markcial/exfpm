defmodule Exfpm.Encoders.Pairs do
    ...
    def pair_length!(data) do
        case packet_size(data) do
            {:error, reason1} -> raise reason1
            {:ok, name_size, data1} -> case packet_size(data1) do
    ...
    """
    def encode(pairs) when is_list(pairs) do
        Enum.map(pairs, &encode/1)
    end
end